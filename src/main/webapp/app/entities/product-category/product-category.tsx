  ...
  return (
    <div>
      <h2 id="product-category-heading" data-cy="ProductCategoryHeading">
        <Translate contentKey="wmmappApp.productCategory.home.title">Product Categories</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="wmmappApp.productCategory.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/product-category/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            ...
            &nbsp;
            <Translate contentKey="wmmappApp.productCategory.home.createLabel">Create new Product Category</Translate>
          </Link>
        </div>
      </h2>
        ...
        {productCategoryList && productCategoryList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="wmmappApp.productCategory.id">ID</Translate>{' '}
                ...
                <th>
                  <Translate contentKey="wmmappApp.productCategory.tiles">Tiles</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {productCategoryList.map((productCategory, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{productCategory.active ? 'true' : 'false'}</td>
                  <td>
                    {productCategory.tiles
                      ? productCategory.tiles.map((val, j) => (
                          <span key={j}>
                            <Link to={`/mechanic-tile/${val.id}`}>{val.title}</Link>
                            {j === productCategory.tiles.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="wmmappApp.productCategory.home.notFound">No Product Categories found</Translate>
            </div>
          )
        )}
      </div>
